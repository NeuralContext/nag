...
def repository_output_path(value: str | None, default_name: str) -> Path:
    """Resolve and prepare a repository-local output path."""
    repo_root = find_repo_root().resolve()
    candidate = Path(value).expanduser() if value else repo_root / "tmp" / default_name
    ...
    try:
        ...
        raise SystemExit(f"error: output path must be inside repository root {repo_root}: {candidate}") from error
    candidate.parent.mkdir(parents=True, exist_ok=True)
    return candidate


def atomic_write_text(path: Path, content: str) -> None:
    """Replace a text artifact only after its complete temporary write."""
    temporary_path = None
    try:
