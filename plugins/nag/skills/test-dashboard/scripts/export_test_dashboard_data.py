...
Writes `<repo-root>/tmp/test-dashboard-data.json` by default, or the
repository-local path selected with `--out`:
...
Usage:
    <detected python> export_test_dashboard_data.py [--out PATH]
"""
import argparse
import json
import os
...
def run_tests_with_contexts(data_file: Path) -> None:
    ...
    if result.returncode != 0:
        raise SystemExit(f"error: instrumented pytest run failed with exit code {result.returncode}")
...
def main() -> None:
    ...
    parser.add_argument("--out", default=None, help="Repository-local output JSON path")
    args = parser.parse_args()
    out_path = _detect.repository_output_path(args.out, "test-dashboard-data.json")
    ...
    _detect.atomic_write_text(out_path, json.dumps(data, indent=1))
